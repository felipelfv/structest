...
draw_vectors <- function(corr, main = "") {
  ...
  text(0.9, -0.18, bquote(r == .(sprintf("%.1f", corr))),
       cex = 0.9, col = "#444")
  text(0.9, -0.28, bquote(theta == .(paste0(deg, "\u00b0"))),
       cex = 0.9, col = "#7d6608")
}
...
lambdas <- c(0.9, 0.75, 0.6)
noise_mag <- c(0.2, 0.28, 0.4)
noise_sign <- c(1, -1, 1)

plot(NULL, xlim = c(-0.15, 1.4), ylim = c(-0.15, 1.15),
...
text(1.33 * eta_dir[1], 1.33 * eta_dir[2],
...
for (i in seq_along(lambdas)) {
  ...
  # labels
  offset <- if (ns == -1) -0.06 else 0.06
  text(ex + 0.05, ey + offset, names3[i], col = colors3[i],
       font = 2, cex = 1.1)

  # lambda label along signal
  lmx <- lam / 2 * eta_dir[1] + 0.04 * (if (ns > 0) -perp_dir[1] else perp_dir[1])
  lmy <- lam / 2 * eta_dir[2] + 0.04 * (if (ns > 0) -perp_dir[2] else perp_dir[2])
  text(lmx, lmy, bquote(lambda[.(i)] * eta), col = colors3[i], cex = 0.8)

  # epsilon label
  emx <- (sx + ex) / 2 + 0.05 * (if (ns > 0) perp_dir[1] else -perp_dir[1])
  emy <- (sy + ey) / 2 + 0.05 * (if (ns > 0) perp_dir[2] else -perp_dir[2])
  text(emx, emy, bquote(epsilon[.(i)]), col = "grey50", cex = 0.8)
}
...
$\boldsymbol{\lambda}\boldsymbol{\lambda}^\top$. The internal geometry
...
```{r fig3-structural-vs-non, echo=FALSE, fig.cap="**Figure 3.** The same indicator fan under two scenarios. **Structural (A):** $Y$ projects only onto $\\eta$; its covariance with each $X_i$ is proportional to $\\lambda_i$. **Non-structural (B):** $Y$ also projects onto $\\varepsilon_3$'s direction, giving $X_3$ an extra association with $Y$ that breaks proportionality --- even though the fan of indicators is unchanged.", fig.width=7, fig.height=4.5}
draw_scenario <- function(structural = TRUE, main = "") {
  eta_angle <- 0.3
  eta_dir <- c(cos(eta_angle), sin(eta_angle))
  perp_dir <- c(-sin(eta_angle), cos(eta_angle))

  plot(NULL, xlim = c(-0.1, 1.5), ylim = c(-0.2, 1.1),
       asp = 1, axes = FALSE, xlab = "", ylab = "", main = main)

  # eta axis
  segments(-0.05 * eta_dir[1], -0.05 * eta_dir[2],
           1.35 * eta_dir[1], 1.35 * eta_dir[2],
           col = "#2471a3", lwd = 1.5, lty = 2)
  text(1.38 * eta_dir[1], 1.38 * eta_dir[2],
       expression(eta), col = "#2471a3", font = 2, cex = 1.1)

  lambdas <- c(0.85, 0.7, 0.6)
  noise_m <- c(0.22, 0.26, 0.38)
  noise_s <- c(1, -1, 1)
  colors3 <- c("#c0392b", "#27ae60", "#8e44ad")
  names3 <- c(expression(X[1]), expression(X[2]), expression(X[3]))
  ...
  if (structural) {
    ...
    text(ys_x + 0.07, ys_y - 0.05, "Y", col = "#2471a3",
         font = 2, cex = 1.3)
    text(1.15, -0.08, expression("Y lies on" ~ eta * "-axis"),
         col = "#2471a3", cex = 0.85)
    text(1.15, -0.15, expression(Cov(epsilon[i], Y) == 0 ~ ~forall * i),
         col = "#2471a3", cex = 0.75)
  } else {
    # structural Y faintly
    arrows(0, 0, ys_x, ys_y, col = "#2471a3", lwd = 1.5, lty = 2,
           length = 0.08)
    text(ys_x + 0.07, ys_y - 0.05, "Y (structural)", col = "#2471a3",
         cex = 0.7, font = 3)
    ...
    text(yn_x + 0.07, yn_y + 0.04, "Y", col = "#e74c3c",
         font = 2, cex = 1.3)
    # deviation line
    segments(ys_x, ys_y, yn_x, yn_y, col = "#e74c3c", lwd = 1.5, lty = 2)

    text(1.15, -0.04, expression("Y tilts toward" ~ epsilon[3]),
         col = "#e74c3c", cex = 0.85)
    text(1.15, -0.11, expression(Cov(epsilon[3], Y) != 0),
         col = "#e74c3c", cex = 0.75)
    text(1.15, -0.18, "Proportionality breaks",
         col = "#e74c3c", cex = 0.75)
  }
...
par(mfrow = c(1, 2), mar = c(3, 4, 2.5, 2))
draw_bars("structural", scaled = TRUE,
          main = "Structural: scaled values constant")
draw_bars("swls", scaled = TRUE,
          main = "Non-structural: proportionality breaks")
```
