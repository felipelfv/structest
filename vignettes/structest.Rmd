...
indicators. The $T_0$ and $T_1$ tests formalise this constraint using
distance metric statistics from the generalised methods of moments framework
(Newey & McFadden, 1994).
...
Let $U_k$ be the $(d \times p)$-dimensional column vector with elements
...
for $z_j = 1, \ldots, p$ and $i = 1, \ldots, d$.  The distance metric
statistic is

$$
T_0 = N\!\left(\frac{1}{N}\sum_{k=1}^{N} U_k^{\!\top}\right)
\Sigma^{-1}
\left(\frac{1}{N}\sum_{k=1}^{N} U_k\right),
$$

where $\Sigma$ is the empirical covariance matrix of $U_k$ after adjusting
for the uncertainty in the estimated reliabilities (see below).
The minimum of $T_0$ over the free parameters converges to a $\chi^2$
...
$\leq 1$, since it factors as $\boldsymbol\alpha\,\boldsymbol\beta^\top$.
...
$\beta_w = \beta_{z_j}$. The distance metric statistic is
...
where $\Sigma$ is the empirical covariance matrix of $U_k$. The minimum of
