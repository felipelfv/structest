...
[`fit_structural()`](https://felipelfv.github.io/structest/reference/fit_structural.md)
are meaningful **only when the structural model is not rejected**. When
the overidentification test yields a large \\p\\-value, the model fits
the data adequately and the parameters describe the latent factor
structure:

- **\\\beta_j\\** measures how much the latent factor \\\eta\\ shifts at
  \\Z\\-level \\j\\ relative to the reference. A significant \\\beta_j\\
  means \\Z\\-level \\j\\ is associated with a different level of the
  latent factor.
- **\\\alpha_i\\** (\\T_1\\ only) gives the relative loading of
  indicator \\i\\ compared to the reference indicator (\\\alpha_1 =
  1\\). Under the structural model, \\\alpha_i = \lambda_i /
  \lambda_1\\.
- **\\\gamma_i\\** is the intercept for indicator \\i\\ at the reference
  level of \\Z\\: \\\gamma_i = \mu_i + \lambda_i\\E(\eta \mid Z =
  z_1)\\.

### When the model is rejected

If the overidentification test rejects (small \\p\\-value), the
structural model does not hold and the parameter estimates should not be
interpreted substantively. Under misspecification, the model is fitting
a rank-1 structure to data that does not have one, so the estimates do
not correspond to well-defined population quantities and the standard
errors are not valid.

In particular, the per-parameter \\p\\-values **cannot** be used to
diagnose *which* indicator breaks the structural interpretation. The
estimates and their standard errors are derived under the assumption
that the model is correct; when it is not, they do not have a meaningful
interpretation.
