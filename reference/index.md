# Package index

## All functions

- [`coef(`*`<structest_fit>`*`)`](https://felipelfv.github.io/structest/reference/coef.structest_fit.md)
  : Extract coefficients from a structest_fit object
- [`confint(`*`<structest_fit>`*`)`](https://felipelfv.github.io/structest/reference/confint.structest_fit.md)
  : Confidence intervals for structest_fit parameters
- [`equiv_test()`](https://felipelfv.github.io/structest/reference/equiv_test.md)
  : Equivalence Test for the Structural Interpretation
- [`estimate_reliability()`](https://felipelfv.github.io/structest/reference/estimate_reliability.md)
  : Estimate Reliability Coefficients (Section 3.1)
- [`fit_structural()`](https://felipelfv.github.io/structest/reference/fit_structural.md)
  : Fit a Structural Model with Standard Errors
- [`print(`*`<structest>`*`)`](https://felipelfv.github.io/structest/reference/print.structest.md)
  : Print method for structest objects
- [`print(`*`<structest_equiv>`*`)`](https://felipelfv.github.io/structest/reference/print.structest_equiv.md)
  : Print method for structest equivalence tests
- [`print(`*`<structest_fit>`*`)`](https://felipelfv.github.io/structest/reference/print.structest_fit.md)
  : Print method for structest_fit objects
- [`structest`](https://felipelfv.github.io/structest/reference/structest-package.md)
  [`structest-package`](https://felipelfv.github.io/structest/reference/structest-package.md)
  : structest: Testing the Structural Interpretation of a Latent Factor
  Model
- [`summary(`*`<structest>`*`)`](https://felipelfv.github.io/structest/reference/summary.structest.md)
  : Summary method for structest objects
- [`summary(`*`<structest_fit>`*`)`](https://felipelfv.github.io/structest/reference/summary.structest_fit.md)
  : Summary method for structest_fit objects
- [`test_t0()`](https://felipelfv.github.io/structest/reference/test_t0.md)
  : T0 Test: Reliability-Dependent Test of Structural Interpretation
  (Section 3.2)
- [`test_t1()`](https://felipelfv.github.io/structest/reference/test_t1.md)
  : T1 Test: Reliability-Independent Test of Structural Interpretation
  (Section 3.3)
- [`vcov(`*`<structest_fit>`*`)`](https://felipelfv.github.io/structest/reference/vcov.structest_fit.md)
  : Variance-covariance matrix for structest_fit parameters
